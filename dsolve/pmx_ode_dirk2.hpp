#ifndef STAN_MATH_TORSTEN_ODE_DIRK2_HPP
#define STAN_MATH_TORSTEN_ODE_DIRK2_HPP
...
namespace torsten {
  ...
  template <typename F, typename Tt, typename T_initial, typename... T_param>
  std::vector<Eigen::Matrix<typename stan::return_type_t<Tt, T_initial, T_param...>,-1,1> >
  pmx_ode_dirk2(const F& f,
                const Eigen::Matrix<T_initial, -1, 1>& y0,
                double t0,
                const std::vector<Tt>& ts,
                std::ostream* msgs,
                const T_param&... args) {
    return pmx_ode_dirk2_ctrl(f, y0, t0, ts, 1.e-10, 1.e-10, 1e6, msgs, args...);
  }
}
#endif
