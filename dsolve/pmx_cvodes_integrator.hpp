...
namespace dsolve {
    ...
    //   CvodesObserver(const Ode& ode0) :
    //     ode(ode0), n(ode.N), m(ode.M),
    //     y{ode.ts_.size(), std::vector<typename Ode::scalar_t>(ode.N, 0.0)},
    //     step_counter_(0)
    //   {}

    //   inline void operator()(const N_Vector& curr_result, double t) {
    //     if(t > ode_.t0_) {
    ...
    //   CvodesDataObserver(const Ode& ode) :
    //     ode_(ode), n(ode.N), m(ode.M),
