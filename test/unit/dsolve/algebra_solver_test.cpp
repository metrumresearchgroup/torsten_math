...
// TEST(simple_eq, newton) {
//   using stan::math::var;
//   const std::vector<double> scaling(2, 1.0);
//   simple_eq_functor f;
//   torsten::nl_system_adaptor<simple_eq_functor> f_nl;
//   Eigen::Matrix<stan::math::var, -1, 1> x_var(2);
//   x_var << 1, 1;
//   Eigen::VectorXd y_dbl(3);
//   y_dbl << 5, 4, 2;
//   std::vector<double> x_r;
//   std::vector<int> x_i;
//   Eigen::MatrixXd sol1 = pmx_algebra_solver_newton(f_nl, x_var, scaling, scaling, nullptr, y_dbl, x_r, x_i);
//   Eigen::MatrixXd sol2(2, 1);
//   sol2 << 20, 2;
//   EXPECT_MATRIX_FLOAT_EQ(sol1, sol2);
// }

TEST(simple_eq, newton_var) {
  using stan::math::var;
  const std::vector<double> scaling(2, 1.0);
  simple_eq_functor f;
  torsten::nl_system_adaptor<simple_eq_functor> f_nl;
  ...
  EXPECT_FLOAT_EQ(sol(0).val(), 20.0);
  EXPECT_FLOAT_EQ(sol(1).val(), 2.0);
  
  // sol(0).grad();
  // EXPECT_FLOAT_EQ(y(0).adj(), 4.0);
  // EXPECT_FLOAT_EQ(y(1).adj(), 5.0);
  // EXPECT_FLOAT_EQ(y(2).adj(), 0.0);
  // nested.set_zero_all_adjoints();
}

// TEST(debug, newton_var) {
//   using stan::math::var;
//   const std::vector<double> scaling(3, 1.0);
//   non_linear_eq_functor f;
//   torsten::nl_system_adaptor<non_linear_eq_functor> f_nl;

//   stan::math::nested_rev_autodiff nested;

//   Eigen::Matrix<stan::math::var, -1, 1> x_var(3);
//   x_var << -3, -3, -3;
//   Eigen::Matrix<stan::math::var, -1, 1> y1(3);
//   y1 << 4, 6, 3;
//   Eigen::Matrix<stan::math::var, -1, 1> y2(3);
//   y2 << 4, 6, 3;
//   std::vector<double> x_r;
//   std::vector<int> x_i;

//   // Eigen::Matrix<stan::math::var, -1, 1> res(3);
//   // {
//   // torsten::PMXNLSystem<torsten::nl_system_adaptor<non_linear_eq_functor>, stan::math::var,
//   //                      Eigen::Matrix<stan::math::var, -1, 1>,
//   //                      std::vector<double> ,
//   //                      std::vector<int> > nl(f_nl, x_var, nullptr, y1, x_r, x_i);
//   // torsten::NewtonSolver s;
//   // res = s.solve(nl, scaling, scaling, 1e-3, 1e-6, 200);
//   // }
//   // std::cout << "taki test res: " << res << "\n";

//   Eigen::Matrix<stan::math::var, -1, 1> sol1 = pmx_algebra_solver_newton(f_nl, x_var, scaling, scaling, nullptr, y1, x_r, x_i);
//   // Eigen::Matrix<stan::math::var, -1, 1> sol2 = stan::math::algebra_solver_newton(f, x_var, y2, x_r, x_i, nullptr);

//   {
//     Eigen::VectorXd sol1_dbl = stan::math::value_of(sol1);
//     // Eigen::VectorXd sol2_dbl = stan::math::value_of(sol2);
//     std::cout << "taki test: " << sol1_dbl << "\n";
//     // EXPECT_MATRIX_FLOAT_EQ(sol1_dbl, sol2_dbl);
//   }

//   // for (auto i = 0; i < 2; ++i) {
//   //   nested.set_zero_all_adjoints();
//   //   sol1(i).grad();
//   //   std::vector<double> g1{y1(0).adj(), y1(1).adj(), y1(2).adj()};
//   //   nested.set_zero_all_adjoints();
//   //   sol2(i).grad();
//   //   std::vector<double> g2{y2(0).adj(), y2(1).adj(), y2(2).adj()};
//   //   EXPECT_FLOAT_EQ(g1[0], g2[0]);
//   //   EXPECT_FLOAT_EQ(g1[1], g2[1]);
//   //   EXPECT_FLOAT_EQ(g1[2], g2[3]);
//   // }
